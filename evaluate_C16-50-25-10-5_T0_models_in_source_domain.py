    ...
    for ID in IDs: 
        ...
        for case in pred_cases:
            ...
            for loss, comp_time, sn, stp, sid in zip(losses, computation_times, series_numbers, series_tps, sample_ids):
                result_list.append([ID, case, float(hps['split_train']), IDs_dict[ID], loss, comp_time, sn, stp, sid])
    
    print('evaluation took {}s'.format(time.time() - start_time))
    
    # csv_name = os.path.join(out_path, 'losses_C16-50-25-10-5_T0_in_source_D_long.csv')
    csv_name = os.path.join(out_path, 'losses_C16-5_T0_in_source_D_short.csv')
    with open(csv_name, 'w', newline='') as csvfile:
        csvwriter = csv.writer(csvfile, delimiter=',', quotechar='|', quoting=csv.QUOTE_MINIMAL)
        csvwriter.writerow(['ID', 'target_case', 'source_train_size', 'model_name', 'source_loss', 'computation_time', 'series_number', 'series_timePoint', 'sample_id'])
